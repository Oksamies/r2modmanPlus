<template>
    <div class="tab-content package-main">
        <div v-if="loading" class="loading-placeholder">Loading changelog...</div>
        <div v-else-if="changelogContent" v-html="changelogContent" class="markdown"></div>
        <div v-else class="empty-placeholder">No changelog available.</div>
    </div>
</template>
...
const fetchChangelog = async (data: PackageListingDetailsResponseData) => {
    ...
    try {
        ...
        const url = `${new URL(activeGame.thunderstoreUrl).origin}/api/cyberstorm/package/${data.namespace}/${data.name}/v/${version}/changelog/`;
        
        const response = await fetch(url);
        if (response.ok) {
            const json = await response.json();
            changelogContent.value = json.html;
        } else {
...
// Markdown styles (duplicated for now, could be shared)
.package-main {
